...
int strbuffer_init(scope strbuffer_t* strbuff)
	...
	do
	{
		...
		strbuff.value = cast(char*)(jansson.jansson_private.jsonp_malloc(strbuff.size));
...
void strbuffer_close(scope strbuffer_t* strbuff)
	...
	do
	{
		jansson.jansson_private.jsonp_free(strbuff.value);
		strbuff.size = 0;
		strbuff.length_ = 0;
		strbuff.value = null;
	}
...
int strbuffer_append_bytes(scope strbuffer_t* strbuff, scope const char* data, size_t size)
	...
	do
	{
		if (size >= (strbuff.size - strbuff.length_)) {
			...
			size_t new_size = mixin (jansson.jansson_private.max!("strbuff.size * STRBUFFER_FACTOR", "strbuff.length_ + size + 1"));

			char* new_value = cast(char*)(jansson.jansson_private.jsonp_malloc(new_size));
