 ...
 * License: MIT
 */
module jansson.test.test_memory_funcs;
...
private void create_and_free_complex_object()
	...
	do
	{
		jansson.jansson.json_t* obj = jansson.pack_unpack.json_pack("{s:i,s:n,s:b,s:b,s:{s:s},s:[i,i,i]}", &("foo\0"[0]), 42, &("bar\0"[0]), &("baz\0"[0]), 1, &("qux\0"[0]), 0, &("alice\0"[0]), &("bar\0"[0]), &("baz\0"[0]), &("bob\0"[0]), 9, 8, 7);
...
private void create_and_free_object_with_oom()
	...
	do
	{
		...
		for (size_t i = 0; i < 10; i++) {
			jansson.jansson_private.snprintf(&(key[0]), key.length, &("%d\0"[0]), i);
			jansson.value.json_object_set_new(obj, &(key[0]), jansson.value.json_integer(i));
		}
	}
...
unittest
{
	...
	jansson.jansson.json_malloc_t mfunc = null;
	jansson.jansson.json_free_t ffunc = null;

	jansson.memory.json_set_alloc_funcs(&.my_malloc, &.my_free);
	jansson.memory.json_get_alloc_funcs(&mfunc, &ffunc);
...
unittest
{
	jansson.test.util.init_unittest();
	.free_called = 0;
	jansson.memory.json_set_alloc_funcs(&.oom_malloc, &.oom_free);
...
{
	jansson.test.util.init_unittest();
	jansson.memory.json_set_alloc_funcs(&.secure_malloc, &.secure_free);
	.create_and_free_complex_object();
}
...
unittest
{
	...
	jansson.test.util.init_unittest();
	jansson.memory.json_get_alloc_funcs(null, null);
}
