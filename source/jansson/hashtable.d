...
{
	enum hash_str = "(cast(size_t)(jansson.lookup3.hashlittle((" ~ key ~ "), (" ~ len ~ "), jansson.hashtable_seed.hashtable_seed)))";
}
...
private int hashtable_do_del(scope .hashtable_t* hashtable_, scope const char* key, size_t key_len, size_t hash)
	...
	do
	{
		size_t index = hash & mixin (jansson.lookup3.hashmask!("hashtable_.order"));
		...
		scope (exit) {
			jansson.jansson_private.jsonp_free(pair);
		}
		...
		jansson.jansson.json_decref(pair.value);
...
private int hashtable_do_rehash(scope .hashtable_t* hashtable_)
	...
	do
	{
		...
		size_t new_size = mixin (jansson.lookup3.hashsize!("new_order"));

		.hashtable_bucket* new_buckets = cast(.hashtable_bucket*)(jansson.jansson_private.jsonp_malloc(new_size * .bucket_t.sizeof));

		if (new_buckets == null) {
			return -1;
		}

		jansson.jansson_private.jsonp_free(hashtable_.buckets);
		hashtable_.buckets = new_buckets;
		hashtable_.order = new_order;

		for (size_t i = 0; i < mixin (jansson.lookup3.hashsize!("hashtable_.order")); i++) {
...
int hashtable_init(scope .hashtable_t* hashtable_)
	...
	do
	{
		...
		hashtable_.buckets = cast(.hashtable_bucket*)(jansson.jansson_private.jsonp_malloc(mixin (jansson.lookup3.hashsize!("hashtable_.order")) * .bucket_t.sizeof));
...
nothrow @trusted @nogc
private .pair_t* init_pair(scope jansson.jansson.json_t* value, scope const char* key, size_t key_len, size_t hash)
	...
	do
	{
		...
		.pair_t* pair = cast(.pair_t*)(jansson.jansson_private.jsonp_malloc(.pair_t.key.offsetof + key_len + 1));
 ...
 */
nothrow @trusted @nogc
int hashtable_set(scope .hashtable_t* hashtable_, scope const char* key, size_t key_len, scope jansson.jansson.json_t* value)
	...
	do
	{
		...
		if (hashtable_.size >= mixin (jansson.lookup3.hashsize!("hashtable_.order"))) {
...
void* hashtable_get(scope .hashtable_t* hashtable_, scope const char* key, size_t key_len)
	...
	{
		size_t hash = mixin (.hash_str!("key", "key_len"));
		.bucket_t* bucket = &hashtable_.buckets[hash & mixin (jansson.lookup3.hashmask!("hashtable_.order"))];
 ...
 */
nothrow @trusted @nogc
void hashtable_iter_set(scope void* iter, scope jansson.jansson.json_t* value)
