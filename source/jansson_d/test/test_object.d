...
unittest
{
	...
	assert((!jansson_d.jansson.json_object_set(object_, "a", ten)) && (!jansson_d.jansson.json_object_set(object_, "b", ten)) && (!jansson_d.jansson.json_object_set(object_, "c", ten)) && (!jansson_d.jansson.json_object_set(object_, "d", ten)) && (!jansson_d.jansson.json_object_set(object_, "e", ten)), "unable to set value");

	assert(jansson_d.value.json_object_size(object_) == 5, "invalid size");
...
unittest
{
	...
	assert(!jansson_d.value.json_object_update(object_, other), "unable to update an empty object with an empty object");

	assert(jansson_d.value.json_object_size(object_) == 0, "invalid size after update");
	...
	assert(!jansson_d.value.json_object_update(object_, other), "unable to update an empty object");

	assert(jansson_d.value.json_object_size(object_) == 5, "invalid size after update");

	assert((jansson_d.value.json_object_get(object_, "a") == ten) && (jansson_d.value.json_object_get(object_, "b") == ten) && (jansson_d.value.json_object_get(object_, "c") == ten) && (jansson_d.value.json_object_get(object_, "d") == ten) && (jansson_d.value.json_object_get(object_, "e") == ten), "update works incorrectly");

	/* perform the same update again */

	assert(!jansson_d.value.json_object_update(object_, other), "unable to update a non-empty object");
	...
	assert(!jansson_d.value.json_object_update(object_, other), "unable to update a nonempty object");

	assert(jansson_d.value.json_object_size(object_) == 8, "invalid size after update");

	assert((jansson_d.value.json_object_get(object_, "a") == nine) && (jansson_d.value.json_object_get(object_, "b") == nine) && (jansson_d.value.json_object_get(object_, "f") == nine) && (jansson_d.value.json_object_get(object_, "g") == nine) && (jansson_d.value.json_object_get(object_, "h") == nine), "update works incorrectly");

	/* update_new check */
	assert(!jansson_d.value.json_object_clear(object_), "clear failed");

	assert((!jansson_d.jansson.json_object_set(object_, "a", ten)) && (!jansson_d.jansson.json_object_set(object_, "b", ten)) && (!jansson_d.jansson.json_object_set(object_, "c", ten)) && (!jansson_d.jansson.json_object_set(object_, "d", ten)) && (!jansson_d.jansson.json_object_set(object_, "e", ten)), "unable to set value");

	assert(!jansson_d.jansson.json_object_update_new(object_, jansson_d.pack_unpack.json_pack("{s:O, s:O, s:O}", &("b\0"[0]), nine, &("f\0"[0]), nine, &("g\0"[0]), nine)), "unable to update_new a nonempty object");
...
unittest
{
...

	{
		...
		assert(jansson_d.value.json_object_size(object_) == 3, "json_object_update_missing_new didn't add new items");

		assert(jansson_d.value.json_integer_value(jansson_d.value.json_object_get(object_, "foo")) == 1, "json_object_update_missing_new updated existing key");
...
unittest
{
...

	{
		...
		assert(jansson_d.value.json_object_size(object_) == 3, "invalid size after update");

		assert(jansson_d.value.json_integer_value(jansson_d.value.json_object_get(object_, "foo")) == 3, "json_object_update_recursive failed to update existing key");
...

	{
		...
		assert(jansson_d.value.json_object_size(object_) == 2, "invalid size after update");

		assert(jansson_d.value.json_object_get(object_, "foo") != null, "json_object_update_recursive removed existing key");

		assert(jansson_d.value.json_integer_value(jansson_d.value.json_object_get(jansson_d.value.json_object_get(object_, "bar"), "baz")) == 3, "json_object_update_recursive failed to update nested value");

		jansson_d.jansson.json_t* barAfter = jansson_d.value.json_object_get(object_, "bar");
...
unittest
{
	...
	assert(!jansson_d.jansson.json_object_set_nocheck(object_, "foo", string_), "json_object_set_nocheck failed");

	assert(jansson_d.value.json_object_get(object_, "foo") == string_, "json_object_get after json_object_set_nocheck failed");

	/* invalid UTF-8 in key */
	assert(!jansson_d.jansson.json_object_set_nocheck(object_, "a\xefz", string_), "json_object_set_nocheck failed for invalid UTF-8");

	assert(jansson_d.value.json_object_get(object_, "a\xefz") == string_, "json_object_get after json_object_set_nocheck failed");

	assert(!jansson_d.value.json_object_set_new_nocheck(object_, "bax", jansson_d.value.json_integer(123)), "json_object_set_new_nocheck failed");

	assert(jansson_d.value.json_integer_value(jansson_d.value.json_object_get(object_, "bax")) == 123, "json_object_get after json_object_set_new_nocheck failed");
...
unittest
{
	...
	assert(jansson_d.value.json_object_iter_next(object_, null) == null, "able to increment a null iterator");
...
unittest
{
	...
	assert(jansson_d.value.json_object_get(object_, "a") == null, "value for nonexisting key");

	assert(!jansson_d.jansson.json_object_set(object_, "a", string_), "unable to set value");

	assert(jansson_d.jansson.json_object_set(object_, null, string_), "able to set null key");

	assert(!jansson_d.value.json_object_del(object_, "a"), "unable to del the only key");

	assert(!jansson_d.jansson.json_object_set(object_, "a", string_), "unable to set value");

	assert(jansson_d.jansson.json_object_set(object_, "a", null), "able to set null value");
	...
	assert(!jansson_d.value.json_object_del(object_, "px"), "unable to delete an existing key");

	assert(!jansson_d.value.json_object_del(object_, "a"), "unable to delete an existing key");

	assert(!jansson_d.value.json_object_del(object_, "lp"), "unable to delete an existing key");

	/* add many keys to initiate rehashing */

	assert(!jansson_d.jansson.json_object_set(object_, "a", string_), "unable to set value");

	assert(!jansson_d.jansson.json_object_set(object_, "lp", string_), "unable to set value");

	assert(!jansson_d.jansson.json_object_set(object_, "px", string_), "unable to set value");

	assert(!jansson_d.jansson.json_object_set(object_, "c", string_), "unable to set value");

	assert(!jansson_d.jansson.json_object_set(object_, "d", string_), "unable to set value");

	assert(!jansson_d.jansson.json_object_set(object_, "e", string_), "unable to set value");
...
unittest
{
	...
	jansson_d.value.json_object_set_new(object_, "lorem ipsum", jansson_d.value.json_integer(3));
	jansson_d.value.json_object_set_new(object_, "dolor", jansson_d.value.json_integer(4));
	jansson_d.value.json_object_set_new(object_, "sit amet", jansson_d.value.json_integer(5));

	/* changing a value should preserve the order */
	jansson_d.value.json_object_set_new(object_, "bazquux", jansson_d.value.json_integer(6));

	/* deletion shouldn't change the order of others */
	jansson_d.value.json_object_del(object_, "dolor");

	/* add a new item just to make sure */
	jansson_d.value.json_object_set_new(object_, "helicopter", jansson_d.value.json_integer(7));
