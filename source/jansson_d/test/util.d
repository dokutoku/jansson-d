...
nothrow @nogc @live
package void check_errors(scope const ref jansson_d.jansson.json_error_t error, jansson_d.jansson.json_error_code_t code_, scope immutable (char)* texts_, size_t num_, scope immutable (char)* source_, int line_, int column_, int position_)

	do
	{
		if (jansson_d.jansson.json_error_code(&error) != code_) {
			//failhdr;
			core.stdc.stdio.fprintf(core.stdc.stdio.stderr, "code: %d != %d\n", jansson_d.jansson.json_error_code(&error), code_);
			assert(false);
		}
...
nothrow @nogc @live
package void check_error(scope const ref jansson_d.jansson.json_error_t error, jansson_d.jansson.json_error_code_t code_, scope immutable (char)* text_, scope immutable (char)* source_, int line_, int column_, int position_)
